...
m_c = pd.crosstab(index=df1["Namera_cepljenja"], columns='count',  margins=True)  
...
# Load the shapefile
#shapefile_path = "../../covid_19_delo/Serban_a/Europe/CNTR_BN_01M_2020_3035.shp"
...
# Display the plot
plt.show()
m_c = pd.crosstab(index=Europe["name"], columns='count',  margins=True)  
print(m_c)
print(Europe["name"])
# list of values of 'Marks' column
marks_list = Europe['name'].tolist()
...
print(Europe.head())
#print(gdf)
Europe.columns=['pop_est' ,'continent' , 'name' ,'Three_Letter_Country_Code' , 'gdp_md_est' , 'geometry']
...
merge=pd.merge(Europe,df1,on='Three_Letter_Country_Code')

#print(merge) 

# plot confirmed cases world map 
merge.plot(column='Zarota_virusi', scheme="quantiles", figsize=(25, 20), legend=True,cmap='coolwarm')
